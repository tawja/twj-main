...
import org.tawja.maven.plugins.frontend.resources.Resource;
import static org.twdata.maven.mojoexecutor.MojoExecutor.*;

/**
 * MOJO executing maven-resources-plugin to create target/{jsTargetDir}
 * directory containing all statics.
 *
 * @author Adam Dubiel
 */
...
public class CreateResourcesMojo extends BaseMavenGruntMojo {
    ...
    /**
     * Change resources plugin version if needed, defaults to 2.7.
     */
    @Parameter(property = "mavenResourcesPluginVersion", defaultValue = "2.7")
     ...
     * workflow, be sure to make Grunt ignore there resources, as it will
    ...
    private Element[] createResourceElements() {
        ...
        Element normalResourcesElement = element(name("resource"),
                element(name("directory"), sourceDirectory + "/" + frontendSourceDirectory),
                element(name("includes"),
                        element(name("include"), "**/*")
                ),
                element(name("excludes"),
                        createResourcesListElement(excludedResources, "exclude",
                                null
                        //element(name("exclude"), "**/" + npmOfflineModulesFile),
                        //element(name("exclude"), "**/" + bowerOfflineComponentsFile)
                        )
                ),
    ...
    private Element[] createResourcesListElement(String[] resources, String elementName, Element... append) {
        Element[] elements = new Element[resources.length + (append != null ? append.length : 0)];
    ...
    private void createInnerPropertiesResource() {
        Resource.from("/" + INNER_PROPERTIES_RESOURCE_NAME, getLog())
                ...
                .withFilter("frontendSourceDirectory", frontendSourceDirectory)
    ...
    private String filteredResourcesAsJSONArray() {
        ...
        builder.append("\"").append("**/").append(bowerOfflineComponentsFile).append("\"").append(", ");
    ...
    private String pathToWatchDirectory() {
        return fullFrontendSourceDirectory();
    }
