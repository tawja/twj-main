...
/**
 * Unpack node_modules.tar and, Run NPM install and  rebuild.
 *
 * @author Adam Dubiel
 */
...
public class ExecNpmOfflineMojo extends ExecNpmMojo {
    ...
    protected List<Executable> getExecutables() {
        unpackModules();
        return Arrays.asList(createNpmInstallExecutable(true), createNpmRebuildExecutable());
    }
    ...
    private void unpackModules() {
        ...
        if (npmOfflineModulesFilePath == null) {
            npmOfflineModulesFilePath = relativeFrontendSourceDirectory();
        }
    ...
    protected Executable createNpmRebuildExecutable() {
