...
public class TwjAutoProcessor {

    /**
     * The property name used for the bundle directory.
    *
     */
    public static final String AUTO_DEPLOY_DIR_PROPERTY = "tawja.auto.deploy.dir";
    /**
    ...
    *
     */
    public static final String AUTO_DEPLOY_ACTION_PROPERTY = "tawja.auto.deploy.action";
    /**
     * The property name used to specify auto-deploy start level.
    *
     */
    public static final String AUTO_DEPLOY_STARTLEVEL_PROPERTY = "tawja.auto.deploy.startlevel";
    /**
    ...
    *
    ...
    private static void processAutoDeploy(Map configMap, BundleContext context) {
        ...
        if (actionList.size() > 0) {
            ...
            BundleStartLevel sl = (BundleStartLevel) context.getService(
                    context.getServiceReference(BundleStartLevel.class.getName()));
    ...
    private static void processAutoProperties(Map configMap, BundleContext context) {
        ...
        for (Iterator i = configMap.keySet().iterator(); i.hasNext();) {
            ...
            if (!key.startsWith(TwjCoreProperties.AUTO_INSTALL_PROP) && !key.startsWith(TwjCoreProperties.AUTO_START_PROP)) {
                continue;
            }
            ...
            int startLevel = sl.getStartLevel();
            if (!key.equals(TwjCoreProperties.AUTO_INSTALL_PROP) && !key.equals(TwjCoreProperties.AUTO_START_PROP)) {
                try {
        ...
        for (Iterator i = configMap.keySet().iterator(); i.hasNext();) {
            String key = ((String) i.next()).toLowerCase();
            if (key.startsWith(TwjCoreProperties.AUTO_START_PROP)) {
                StringTokenizer st = new StringTokenizer((String) configMap.get(key), "\" ", true);
                for (String location = nextLocation(st); location != null; location = nextLocation(st)) {
